    ...
    for city in merged:
        for place in merged[city]:
            if place in latest:
                ...
                merged[city][place] = _pick_better(merged[city][place], latest[place])
    if merged_places:
...
class Accordion(Kunto):
    ...
    def load(self):
        ...
        r.raise_for_status()
        data = self._parse_html(r.text, parse_opts=self.cfg["html_parser_opts"])
    ...
    def _parse_html(cls, text: str, parse_opts: dict):
        ...
        for city_list in tree.xpath('//*[@id="%s"]/div' % acc_label):
            try:
                city = city_list.xpath("preceding-sibling::h3[1]/a")[0].text.strip()
            except IndexError:
                continue
            data[city] = {}
            positions = city_list.xpath("div/span[2]")
            for pos in positions:
                data[city][pos.text.strip()] = cls._parse_status_from_element(pos)
...
class Latest(Kunto):
    ...
    def load(self):
        ...
        try:
            ...
            logger.debug("failed to load latest for %s, %s", self.sport, self.area)
            return {}
    ...
    def _parse_html(cls, text: str):
        ...
        for div in root.iterfind(".//div"):
            ...
            if place is not None and update is not None:
                data[place.text.strip()] = cls._parse_status_text(update.text.strip())
